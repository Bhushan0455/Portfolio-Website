...
export default function FounderPhilosophy() {
  ...
  const imageVariants = {
    ...
    visible: {
      opacity: 0.82, // Reduced overlay block: portrait itself is highly visible (82% opacity)
      x: 0,
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_30%_50%,rgba(15,39,68,0.45)_0%,rgba(7,22,44,1)_100%)] pointer-events-none z-0" />
      ...
      {/* ── Right third speaking portrait with subtle parallax ── */}
      <motion.div
        variants={imageVariants}
        initial="hidden"
        whileInView="visible"
        viewport={{ once: true, margin: '-50px' }}
        className="absolute right-0 top-0 bottom-0 w-full lg:w-[40%] pointer-events-none z-0 overflow-hidden select-none"
      >
        <motion.img
          src={founderSpeakingImg}
          alt="Priyanshu Chauhan speaking on stage"
          style={{ y: yParallax }}
          className="absolute -top-[15%] w-full h-[130%] object-cover object-left lg:object-center brightness-[0.88] contrast-[1.05]"
        />
        {/* Soft edge-blending gradients allowing lights and crowd details to show through */}
        <div className="absolute inset-0 bg-gradient-to-r from-[#07162c] via-[#07162c]/40 to-transparent" />
